...
Preserve supported visible concerns. For each image, independently inspect peer consistency and the composition of all supporting choices and the main action. A shared row can still be fragmented by large gaps between related choices; compare the actual grouping before recommending more space or dividers. If the draft describes a visible composition concern but omits it from findings or candidates, add a distinct concern for that image. A cross-screen position difference does not replace a within-screen composition concern. Do not treat a task-structure criticism as established when its only support is an unverified control role or a preference against a plausible conventional order. Put unresolved hypotheses in limitations. Zero concerns is valid. Do not claim a live browser check.
